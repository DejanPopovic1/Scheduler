...
namespace Scheduler.Models
{
    public static class BookingHelper
    {
        ...
        //    List<Booking>[] bookingsByDay = SchedulesSplitter.GetSplitSchedules(DateTime.Now, bookings);//<-- error is here
        //    int i = 0;
        //    foreach (var booking in bookingsByDay) 
        //    {
        //        List<Vertex> schedules = booking.Select(x => { 
        //            ScheduleModel scheduleModel = new ScheduleModel(x.PickupDateTime.TimeOfDay, x.PickupDateTime.TimeOfDay + travelTimes[i]);
